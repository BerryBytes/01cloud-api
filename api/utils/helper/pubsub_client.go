...
func NewPubsubRoutine() (*PubsubRoutine, error) {
	client, err := NewPubsubClient()
	if err != nil {
		log.Warnf("Client not initialized: %v", err)
		return nil, err
	}
...
func (d *PubsubRoutine) Subscribe(topic string, method func(interface{}) error) error {
...
func (d *PubsubRoutine) Publish(topic string, req interface{}) error {
	topic = fmt.Sprintf("%s-%s", os.Getenv("GCLOUD_NAMESPACE"), topic)
	id, err := d.Client.Publish(topic, req)
	if err != nil {
		return err
	}
	log.Debugf("Published message with ID: %s", id)
	return nil
}
...
func NewPubsubClient() (*Client, error) {
	...
	if err != nil {
		log.Warnf("Could not create pubsub Client: %v", err)
	}
	return &Client{
		Client: pubsubClient,
	}, err
...
func (c *Client) CreateTopicIfNotExists(topic string) *pubsub.Topic {
	...
	if err != nil {
		log.Warnf("Failed to create the topic: %v", err)
	}
	return t
}
...
func (c *Client) ReceiveMessage(name string, release func(input interface{}) error) error {
	...
	err := sub.Receive(cctx, func(ctx context.Context, msg *pubsub.Message) {
		data := msg.Data
		log.Printf("Got message: %s :: %s", name, data)
		msg.Ack()
		err := release(data)
		if err != nil {
			log.Error(err)
